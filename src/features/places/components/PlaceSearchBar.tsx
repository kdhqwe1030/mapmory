...
export function PlaceSearchBar({
  ...
  const search = async (q: string) => {
    ...
    try {
      ...
      const coordParams = currentPos
        ? `&lat=${currentPos.lat}&lng=${currentPos.lng}`
        : "";
      const res = await fetch(
        `/api/search?q=${encodeURIComponent(q)}${coordParams}`,
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const coordParams = currentPos
        ...
        : "";
      const res = await fetch(
        `/api/search?q=${encodeURIComponent(query)}${coordParams}`,
      );
      const data = await res.json();
      const items: SearchResult[] = (data.items ?? []).map(
        (item: Record<string, string>) => ({
  ...
  return (
    ...
    >
      ...
      {isOpen && results.length > 0 && (
        <div className="mt-2 bg-white rounded-2xl shadow-[0_4px_20px_rgba(58,46,42,0.12)] max-h-75 overflow-y-auto">
          {results.map((item, i) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-start gap-3 px-4 py-3 hover:bg-brand-cream transition-colors text-left"
              >
                <LocationOnRounded
                  sx={{
                    fontSize: 18,
                ...
                {dist && (
                  <span className="text-xs text-[#C4B4AE] shrink-0 mt-0.5">
                    {dist}
                  </span>
                )}
              </button>
