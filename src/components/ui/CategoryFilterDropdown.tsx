...
export function CategoryFilterDropdown({ categories, value, onChange }: CategoryFilterDropdownProps) {
  ...
  const selected = value === null ? null : categories.find((c) => c.id === value)
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <div
        ...
        className="absolute right-0 top-full mt-2 min-w-[120px] bg-white rounded-2xl shadow-[0_4px_20px_rgba(58,46,42,0.12)] overflow-hidden z-10"
      >
        {/* 전체 옵션 */}
        <button
        ...
        >
          전체
          {value === null && <CheckRounded sx={{ fontSize: 14, color: '#4CAF82' }} />}
        </button>
        {categories.map((cat) => (
          <button
            key={cat.id}
          ...
          >
            ...
            {value === cat.id && <CheckRounded sx={{ fontSize: 14, color: '#4CAF82' }} />}
          </button>
        ))}
      </div>
    </div>
